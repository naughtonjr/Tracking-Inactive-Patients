...
generatetools <- function(master_umb){
  ...
  for(partner in (unique(master_umb$IMPLEMENTING_PARTNER))){
    ...
    for(counter in unique(container$STATE)){
      ...
      # Set Column Widths
      setColWidths(wb, counter, cols = 1:35, widths = "auto")
      
      # Create Instruction Titles
      writeData(wb, counter, "Instructions: Please complete the cells in columns \"AA\" through \"AI\". 
      ...
      addStyle(wb, counter, entry_style_h2, rows = 3, 
               cols = 27:35)
      addStyle(wb, counter, entry_style_cells, rows = 4:(nrow(list_of_states[[counter]])), cols = 27)
      addStyle(wb, counter, entry_style_cells, rows = 4:(nrow(list_of_states[[counter]])), cols = 28)
      addStyle(wb, counter, entry_style_cells, rows = 4:(nrow(list_of_states[[counter]])), cols = 29)
      addStyle(wb, counter, entry_style_cells, rows = 4:(nrow(list_of_states[[counter]])), cols = 34)
      addStyle(wb, counter, entry_style_cells, rows = 4:(nrow(list_of_states[[counter]])), cols = 35)
      # Merge Title Cells
      mergeCells(wb, counter, cols = 1:26, rows = 1:2)
      mergeCells(wb, counter, cols = 27:35, rows = 1)
      
      setRowHeights(wb, counter, rows = 2, heights = 90)
      
      # Apply Data Validation
      dataValidation(wb, counter, col = 27, rows = 4:(nrow(list_of_states[[counter]])), type = "list", value = "'dropdown list'!$G$7:$G$10") #LOST_IN_TX (Y)
      dataValidation(wb, counter, col = 28, rows = 4:(nrow(list_of_states[[counter]])), type = "list", value = "'dropdown list'!$J$7:$J$8") #TRACKING_ATTEMPTED (Y)
      dataValidation(wb, counter, col = 29, rows = 4:(nrow(list_of_states[[counter]])), type = "list", value = "'dropdown list'!$K$7:$K$8") #REACHED(Y)
      dataValidation(wb, counter, col = 34, rows = 4:(nrow(list_of_states[[counter]])), type = "list", value = "'dropdown list'!$L$7:$L$11") #NOT_REACHED_REASON (Y)
      ...
      # Set Column Widths
      setColWidths(wb, counter, cols = c(1:26), widths = 15)
      setColWidths(wb, counter, cols = c(27:35), widths = 25)
    }
    ...
    saveWorkbook(wb, paste("./New Tools/",partner, "_LTFUTrackingToolV2_", date, ".xlsx", sep = ""), overwrite = TRUE)
