...
ndr_wrangle <- function(newLTFU_df, prevLTFU_df, date_data_pull){
  ...
  df2 <- prevLTFU_df %>%
    mutate(DATA_PULL = as.POSIXct(data_pull_date, tz = "UTC")) %>% # "2020-07-05"
...
archive_determine <- function(df){
  ...
  write.xlsx(df1, 
             paste("./Historical_LTFU/Historical_LTFU_Archive_", data_pull_date, ".xlsx", sep = ""), 
             asTable = TRUE)
}
...
continue_determine <- function(df){
  ...
  write.xlsx(df2, 
             paste("./Continue_LTFU/Continue_LTFU_", data_pull_date, ".xlsx", sep = ""), 
             asTable = TRUE)
  return(df2)
}
...
#setwd("/Partner Submissions") # Folder of partner submissions
import_partnersubmissions_old <- function(){
...
merge_ndrdf_partnersub <- function(ndr_df, partnersub){
  ...
  master_umb <- merge(ndr_df, 
                      ...
                      all.x = T)%>%
    mutate(INACTIVE_MONTH = lubridate::month(INACTIVE_DATE, label = T, abbr = T)) %>%
